...
@section('content')
<h3 class="mb-3">Cập nhật loại sản phẩm</h3>
@if (session('message'))
<div class="alert alert-danger">{{ session('message') }}</div>
@endif
<form action="{{ route('cate2.update', $category->cateid) }}" method="post" class="w-50 p-4 shadow">
    @csrf
    <label for="f-catename">Tên Loại Sản Phẩm</label>
    <input type="text" name="catename" id="f-catename" class="form-control m-2"
